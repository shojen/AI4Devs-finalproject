...
[`docs/workflow.md`](../../../docs/workflow/task-files-links-and-ordering.md#regenerating-the-task-coordination-files).
...
- One `##` section per top-level `docs/` folder that has real content, using two link styles depending on what the content demands:
  - **Mandatory** — only *small* hubs every agent needs regardless of task (e.g. `contracts.md`, `workflow.md`, `conventions/base-standards.md`, each a binding-core summary): `@docs/contracts.md`. An `@` import loads the whole file into **every** session, so never `@`-import a large doc, a glob, or a folder.
  - **Conditional** — detail needed only for a specific kind of task: a **plain path** (no `@`) in a table row, e.g. `| need the schema | docs/database/schema.md |`, so it is read on demand.
- List a split doc by its hub (`docs/architecture/authorization.md`), never by its parts; the hub routes the reader to the part.
...
- **Split with a hub + parts, mechanically.** A doc above ~35k characters becomes a hub (intro + a `Table of Contents` table: part link, *Read when*, section names) plus `<doc-stem>/<part>.md` files holding the original sections byte-for-byte. Heading names never change; rewrite every inbound `file.md#anchor` (markdown links and `docs/...` mentions in code comments, `ai-spec/`, `.claude/`), then re-run a link check and confirm no new broken reference. When syncing content into a split doc, edit the **part**; touch the hub table only when a section is added, removed or re-scoped. See `docs/contracts/token-and-doc-rules.md` item 5 of the Doc Growth Management Rule.
