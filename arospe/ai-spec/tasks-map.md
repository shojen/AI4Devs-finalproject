...
As of this snapshot, `ai-spec/tasks/in-progress/` is empty again — `0041-customers-crud-backend.md`
...
for what its closure unblocked). Every other task is either `done/` (closed, merged) or still
sitting directly in `ai-spec/tasks/` (not started). One additional file,
...
- **95 files total**: 94 numbered user stories (54 `done/`, 40 still in `ai-spec/tasks/`) + 1
...
- [Inventory](#inventory)
  - [Done (54) — shipped, out of scope for this graph](#done-54--shipped-out-of-scope-for-this-graph)
  - [Pending — not started (40 numbered + 1 infra doc)](#pending--not-started-40-numbered--1-infra-doc)
...
### Done (54) — shipped, out of scope for this graph
...
### Pending — not started (40 numbered + 1 infra doc)
...
flowchart LR
    ...
    class P0039,P0044,P0045,P0046,P0047,P0048,P0049,P0050,P0051,P0052,P0053,P0054,P0055,P0056,P0057,P0060,P0061,P0062,P0063,P0064,P0065,P0066,P0067,P0069,P0070,P0071,P0072,P0073,P0074,P0075,P0076,P0077,P0078,P0079 pending;
    class P0038,P0042,P0043,P0058,P0059,P0068 ready;
```
...
- **0043 — Customers "new customer" notification (backend).** Same release: `0041` was its only
  dependency and is now `done/`. Touches a new `create_notifications_table` migration,
...
**0038, 0042, 0043 and 0068 are fully independent of each other and of 0058/0059** — no shared
files, no shared tables, and none of them appears in the other's `conflict_risk_with` set in
[`ai-spec/tasks-status.json`](tasks-status.json) (0042's own `conflict_risk_with` names only the
still-blocked `0045`, which is not in this ready set today). All can be dispatched to parallel
...
**0037 and 0041 already closed** (each went dependency-ready the moment its own last blocker
shipped, was claimed via `tasks-status.json` by whichever session picked it up next, and is now in
`done/` too) — a real, worked instance, twice over, of why the JSON registry exists alongside this
...
freed 0042 and 0043 above** — both were `blocked` in the immediately preceding snapshot and moved
to `ready` in this same regeneration pass, per [workflow.md](../docs/workflow.md#regenerating-the-task-coordination-files)'s
...
Once those land, the same "ready" property propagates outward in a few places — **0037** and
**0041** already did (per the note above); **0044** will do the same the moment **both 0042 and
0043** close; the same will also happen for **0060** the moment **0059** closes,
...
1. **Customers → Orders (Epic 3).** `0041` (`done/`) is what unblocked `{0042, 0043}`, both `ready`
   above; `{0042, 0043} → 0044` next. Then `0045` depends on the now-satisfied `0041` plus the
   still-pending payment story `0038` (`0036`, its former third blocker, is now `done`,
