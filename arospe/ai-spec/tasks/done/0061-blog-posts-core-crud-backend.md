...
- **Open, not fixed (spec-level):** `Published` accepts a future date (D-6 says `nullable|date`); `published_at` is a `TIMESTAMP`, so a date past 2038 is a raw error rather than a validation message; an "empty-looking" body such as `<p><br></p>` passes `required`; the tag list has no size cap. None blocks this story; 0063 should cap the tag field and may want the body emptiness check.
