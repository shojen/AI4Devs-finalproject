...
_Last updated: 2026-09-16 — Story 0048 (Order line-item editing backend). Added [Order editability — the second state-based refusal](#order-editability--the-second-state-based-refusal-and-the-shape-three-more-stories-copy), extending [A domain invariant is not an authorization rule](#a-domain-invariant-is-not-an-authorization-rule-and-does-not-live-here) with its second real instance: `App\Exceptions\OrderNotEditableException` (409), a direct `throw` from each of `AddOrderItem`/`RemoveOrderItem`/`UpdateOrderItemQuantity` when `orders.status` is `Shipped`/`Delivered`, checked strictly after `Gate::authorize('update', $order)` (D-6) and re-verified a second time inside the transaction under `lockForUpdate()` (Phase 4 finding F-4) — named as the shape stories 0050–0052 should each copy rather than re-derive. Corrected [`OrderPolicy`'s ability table and its own "three of the four abilities have no caller" claim](#orderpolicy--the-twelfth-policy) in place: `update` now has three real callers (this story), leaving only `viewAny`/`delete` uncalled. Also corrected [What is deliberately not logged](#what-is-deliberately-not-logged)'s `RoleInUseException`/`ImmutableRoleException` bullet, whose "extending the pattern there is deferred" framing this story falsifies: Orders' own state-based refusal, unlike `Role`'s, **does** log.

_Previously: 2026-09-14 — Story 0045 (Orders core CRUD backend). Added [`OrderPolicy` — the twelfth policy](#orderpolicy--the-twelfth-policy): four flat abilities gating the already-seeded `orders.*` permissions, no per-target branch, modelled directly on `ShippingRatePolicy` (this story's exact structural analogue — backend-only, no route, no component). Only `create` has a real caller (`App\Actions\Orders\CreateOrder`, self-authorizing through `LogRefusedPrivilegedAttempt`); `viewAny`/`update`/`delete` ship with no caller at all, deliberately, so stories 0048–0052's future row-state rules (a shipped order's line items may not be edited, a refund refused outside `Paid`/`PartiallyRefunded`) each add a branch to `update()`'s/`delete()`'s existing body rather than relocating a call site. **D-13 is a Phase 2 reversal**, recorded in full: the story's own first draft argued against creating a policy at all, citing `CustomerPolicy`'s D-12 backwards (as a "no policy" precedent, when 0041's D-12 is the record of that policy being *created*) — `ShippingRatePolicy` is what actually settles the question. Updated the overview line, the running "sole outlier among **N** policies" count (to twelve), and `ProductVariantPolicy`'s own absent-policy ordinal (now *thirteenth*, not *ninth*).

_Previously: 2026-09-11 — Story 0044 (Customers — list + create/edit UI). `App\Livewire\Customers\Index` and `routes/customers.php` are the future callers `CustomerPolicy`'s `viewAny`/`delete()` rows previously named as still-unshipped — the policy's own ability table and its "why this policy has real call sites" paragraph are corrected in place (quoted rather than silently rewritten, per this project's audit-authored-page convention) to name the real, shipped component/route rather than a future story. Added a ✅ confirming instance to [the sidebar registry section](#the-second-half-of-a-module-gate-the-sidebar-registry): `items.customers` (`group: null, cluster: null`, the bare top-level shape `items.users` already uses) is Epic 3's first module gate and costs the same "append data, never behavior" the pattern has held since 0013. Added a ✅ note to [step-up authentication's "what it deliberately does not" section](#what-it-protects-and-what-it-deliberately-does-not) recording that this screen adopts no step-up layer at all (D-7) and why — no role, no account status, no authentication-identifier email, no privilege tier to escalate through. No policy, route middleware or Gate mechanism changed; this story consumes all four `CustomerPolicy` abilities and adds none.

_Previously: 2026-09-10 — Story 0042 (Customers — soft delete, backend). `CustomerPolicy` gains a fourth ability, `delete()` — flat and tier-free like its three siblings, no privilege-tier or ownership branch, since a customer holds no privilege to escalate through (D-3) — added to 0041's existing file rather than a new policy class, discharging that story's own named hand-off. 0041's method-existence unit test (which pinned `delete()` as deliberately absent) is updated in place, not deleted or worked around. Corrected a now-stale parenthetical under `CustomerPolicy`'s `update()`/`delete()` write-up: `Customer` now uses `SoftDeletes` (this story), so the claim that it "uses neither `HasRoles` nor `SoftDeletes`" no longer holds — quoted and corrected per this project's audit-authored-page convention, with a pointer to why `SoftDeletes` on `Customer` carries no authentication weight. Fixed a pre-existing stale Table-of-Contents entry that still read "the tenth policy" for `CustomerPolicy` (the heading itself already said "eleventh"), and widened the `VIEW_PERMISSION`/`CREATE_PERMISSION`/`EDIT_PERMISSION` constants sentence to include `DELETE_PERMISSION`. Policy **count** is unchanged at eleven — this story edits an existing policy, it does not add one. Earlier history (story 0041 and before) folded per [contracts.md](../contracts.md#doc-growth-management-rule) — see git history if needed._
