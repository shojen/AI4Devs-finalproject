...
- [Policies](#policies)
  ...
  - [`CustomerPolicy` — the eleventh policy, and the first a passive record with no privilege tier of its own](#customerpolicy--the-eleventh-policy-and-the-first-a-passive-record-with-no-privilege-tier-of-its-own)
  - [`OrderPolicy` — the twelfth policy](#orderpolicy--the-twelfth-policy)
  - [The blog policies — `BlogCategoryPolicy` and `BlogTagPolicy`, the thirteenth and fourteenth](#the-blog-policies--blogcategorypolicy-and-blogtagpolicy-the-thirteenth-and-fourteenth)
...
- [A domain invariant is not an authorization rule, and does not live here](#a-domain-invariant-is-not-an-authorization-rule-and-does-not-live-here)
  - [Order editability — the second state-based refusal, and the shape three more stories copy](#order-editability--the-second-state-based-refusal-and-the-shape-three-more-stories-copy)
  - [Order status regression confirmation — the app's second "are you sure" mechanism](#order-status-regression-confirmation--the-apps-second-are-you-sure-mechanism-and-why-it-is-not-step-up)
  - [The order detail screen — the first three-ability screen](#the-order-detail-screen--the-first-three-ability-screen)
...
| `Super Admin` | `web` | **0 of 43** | the [`Gate::before` bypass](#the-super-admin-bypass) |
...
- **In `DeleteProductCategory` specifically, the authorization call runs *before* the in-use product count, and the order is load-bearing.** The in-use block ([schema.md § `product_categories`](../database/schema-products.md#product_categories)) is a domain invariant, not an authorization rule — it stays a `ValidationException`, not a `Gate` denial, exactly as [A domain invariant is not an authorization rule](#a-domain-invariant-is-not-an-authorization-rule-and-does-not-live-here) describes for `SalesRegionPolicy`'s default-region rule. But the count itself is data, and disclosing it to an actor who does not even hold `products.delete` would leak information the `Gate` refusal exists to withhold — so the `delete` gate must be asked, and must fail, before the count is ever computed. Reversing the order would turn a plain 403 into a business message ("this category is used by 12 products") available to anyone who can reach the method.
...
| `update` | `(User $actor, Order $target)` | holds `orders.edit` — `$target` is **not** consulted today | `App\Actions\Orders\AddOrderItem` / `RemoveOrderItem` / `UpdateOrderItemQuantity` (story 0048), each self-authorizing through `LogRefusedPrivilegedAttempt::authorize()` as their own first check — strictly before their own state-based hard block, see [Order editability](#order-editability--the-second-state-based-refusal-and-the-shape-three-more-stories-copy) below |
...
- **The permission check runs strictly before the state guard, never the reverse.** All three actions call `Gate::authorize('update', $order)` (via `LogRefusedPrivilegedAttempt::authorize()`, against `OrderPolicy::update()`, which gates on `orders.edit` alone) as their first statement, and only then run the hard-block guard. An actor lacking `orders.edit` who targets a `Shipped` order receives the `AuthorizationException`, never the 409 — inverting it would let the 409 disclose the order's existence and shipped/delivered state to somebody with no permission to read either, the identical [step-up ordering rule](#ordering-the-permission-refusal-always-wins) restated a third time on this page.
- **It must bind a Super Admin, so it cannot be a `Gate` check** — the same reasoning as [A rule that must bind a Super Admin actor cannot go through `Gate`](#a-rule-that-must-bind-a-super-admin-actor-cannot-go-through-gate) and [security/authorization-patterns.md](../security/authorization-patterns.md#a-rule-that-must-bind-a-super-admin-actor-must-be-a-direct-throw-not-a-gate-check), arriving from the domain-invariant direction rather than the privilege direction: `Gate::before` grants a Super Admin before `OrderPolicy::update()` ever runs, so a `Gate`-mediated block would be inert against exactly the actor a hard block with "no confirmation path around it" (PRD §3.2) exists to bind. Pinned by a dedicated test asserting a Super Admin is refused identically to an ordinary `orders.edit` holder.
...
This is the `Gate::allows()`-is-a-UI-hint section's ([above](#gateallows-in-a-list-query-is-a-ui-hint-not-a-layer)) first screen with a **state** dimension as well as a permission one, and it renders the two differently on purpose:
...
> having one — [step-up authentication](#what-it-protects-and-what-it-deliberately-does-not) is a separate
...
_Last updated: 2026-09-24 — Story 0059 (Blog tags — backend). Added [The blog policies](#the-blog-policies--blogcategorypolicy-and-blogtagpolicy-the-thirteenth-and-fourteenth) — `BlogTagPolicy`, and `BlogCategoryPolicy` (story 0058), which this page had not documented — and recounted the policy roster from twelve to fourteen (`ls app/Policies`). The section records the app's first branch-dependent ability (`FindOrCreateBlogTag`: gate on `create` or `viewAny`, then `create` again only when it would insert), why the existence-oracle objection does not apply, and why there is one policy per model rather than a shared `BlogPolicy`._

_Previously: 2026-09-23 — Story 0055 (orders list + detail/editor UI). Added [The order detail screen — the first three-ability screen](#the-order-detail-screen--the-first-three-ability-screen) (permission vs state dimensions rendered differently; the Super Admin/Cancel drift), gave `OrderPolicy::viewAny` its Orders callers, marked the 0049/0050 UI-hint forward references as shipped, and recorded the fourth reader of the line-item block (`Order::isLineItemEditable()`/`isRefundable()`). Earlier history folded: 0051 took the catalog to 43 permissions via `ORDER_PERMISSIONS` (`orders.refund`); 0050 added `OrderPolicy::cancel` (two permissions plus the state clause) and its section; 0049 added `transitionStatus` and the regression-confirmation section; 0048 added the order-editability section. Each is described in its own section above._
