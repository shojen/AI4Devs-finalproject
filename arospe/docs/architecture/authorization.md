...
| Ability | Signature | Rule | Authorized from |
| --- | --- | --- | --- |
| `viewAny` | `(User $actor)` | holds `customers.view` | Nothing yet — 0044's `mount()` is its first and only planned caller |
| `create` | `(User $actor)` | holds `customers.create` | `App\Actions\Customers\CreateCustomer::__invoke()`, its own first statement (class-level — no instance exists yet) |
| `update` | `(User $actor, Customer $target)` | holds `customers.edit` — `$target` is **not** consulted | `App\Actions\Customers\UpdateCustomer::__invoke()`, its own first statement |
| `delete` | `(User $actor, Customer $target)` | holds `customers.delete` — `$target` is **not** consulted | Nothing yet — story 0044's future delete control is its first planned caller; asserted directly against the Gate today (`tests/Feature/Customers/DeleteAuthorizationTest.php`), the same "this story ships no route or component" shape `create`/`update` are already in below |
...
**Why this policy has real call sites despite the story shipping no route or component — the identical shape [`ShippingRatePolicy`](#shippingratepolicy--the-ninth-policy-and-the-first-with-real-call-sites-from-day-one) already establishes.** [D-9's reconciling rule](#shippingzonepolicy--the-eighth-policy-and-the-one-d-9-uses-to-reconcile-the-policy-vs-permission-check-divergence-with-0035) applies on its stronger half here: `create`/`update` each have **no route and no Livewire component at all** yet, so the action is the *only* reachable enforcement point that exists today, and each self-authorizes as its own first statement through a constructor-injected `App\Actions\Auth\LogRefusedPrivilegedAttempt::authorize(...)`, `targetType: 'customer'` passed explicitly (`resolveTarget()` auto-resolves only `User`/`Role`). `viewAny` satisfies the *other* half — no caller yet at all — with [story 0044](../../ai-spec/tasks/0044-customers-list-create-edit-ui.md) named as its future consumer, matching how `ShippingRatePolicy::viewAny` waited for story 0037.
...
_Previously: 2026-09-10 — Story 0042 (Customers — soft delete, backend). `CustomerPolicy` gained a fourth ability, `delete()` — flat and tier-free like its three siblings, no privilege-tier or ownership branch, since a customer holds no privilege to escalate through (D-3) — added to 0041's existing file rather than a new policy class, discharging that story's own named hand-off. Policy **count** was unchanged at eleven — that story edited an existing policy rather than adding one. Folded this footer's own prior chain (story 0041's `CustomerPolicy` addition, and everything before it) into this single line, per [contracts.md](../contracts.md#doc-growth-management-rule)'s doc-growth-management rule — every fact that still matters is already stated in the body's own per-section history; see git history for the full prior chain if needed._
