...
- [Policies](#policies)
  ...
  - [`CustomerPolicy` — the tenth policy, and the first a passive record with no privilege tier of its own](#customerpolicy--the-tenth-policy-and-the-first-a-passive-record-with-no-privilege-tier-of-its-own)
...
**`delete()` is deliberately absent, and this is a named hand-off rather than a gap.** Story 0042 (soft delete) adds `delete()` to this same file — it does not create a new policy class — and 0041 pins a unit test asserting `CustomerPolicy` defines exactly `viewAny`/`create`/`update` and **not** `delete()`, in both directions, so a later audit reads the omission as deliberate rather than re-raising it as a finding.
...
**Why `update()` ignores `$target` even though a customer *is* a full-CRUD entity, unlike the read-mostly catalogs (`SalesRegion`, `ShippingZone`) this shape was first built for.** A customer record can never be the acting user and holds no privilege tier at all (`Customer` implements no auth contract, uses neither `HasRoles` nor `SoftDeletes` — story 0041's own D-11), so "may this actor edit this customer" reduces to "does the actor hold `customers.edit`" with zero row-level nuance regardless of which customer is named — there is no Administrator-tier-style branch a `Customer` target could ever trigger the way a `User` target can on `UserPolicy::update()`. The `$target` parameter is kept anyway (not dropped from the signature) for the same two reasons [`SalesRegionPolicy`](#salesregionpolicy--the-third-policy-and-the-first-with-no-target-branch) keeps its own: a future per-row `Gate::allows('update', $customer)` UI hint asks the *identical* method the write authorizes with, so a hint can never drift from a click; and if a later story does introduce a row-level rule (a customer an actor may not view, a per-region restriction), it edits one method body rather than relocating every call site's target — evaluated against a freshly re-fetched row, never the caller's instance, per that policy's own Phase 4 note.

`VIEW_PERMISSION`/`CREATE_PERMISSION`/`EDIT_PERMISSION` are `public const` on the class, per [naming.md](../conventions/naming.md#permission-names)'s convention — the **tenth** follower of it (`UserPolicy` remains the sole outlier among eleven policies).
...
_Last updated: 2026-09-10 — Story 0041 (Customers CRUD backend). Added [`CustomerPolicy` — the eleventh policy, and the first a passive record with no privilege tier of its own](#customerpolicy--the-eleventh-policy-and-the-first-a-passive-record-with-no-privilege-tier-of-its-own): three abilities (`viewAny`/`create`/`update`) gating the already-seeded `customers.*` permissions, no target-dependent branch, modelled on `SalesRegionPolicy`. `create`/`update` self-authorize from `App\Actions\Customers\CreateCustomer`/`UpdateCustomer` as their own first statement, the identical `ShippingRatePolicy`-established shape for a story shipping no route or component; `viewAny` is the one genuinely callerless ability, with story 0044 named as its future consumer. `delete()` is deliberately absent — a named hand-off to story 0042, which adds it to this same file rather than creating a new policy. Updated the "sole outlier among **N** policies" bullets to eleven and corrected `ProductVariantPolicy`'s own absent-policy ordinal (a real eleventh policy now exists, so a hypothetical `ProductVariantPolicy` would be the twelfth, not ninth, absent one).

_Previously: 2026-09-10 — Story 0038 (Payment methods — bank transfer with a validated IBAN, backend). Added [`PaymentMethodPolicy` — the tenth policy](#paymentmethodpolicy--the-tenth-policy-and-the-one-whose-sole-writer-was-corrected-mid-audit-rather-than-designed-self-authorizing-from-phase-1): four abilities gating the seeded `payment-methods.*` permissions, no target-dependent branch, `create()`/`delete()` permanent `false` refusals (with a documented Super Admin `Gate::before` exception to that refusal, Phase 4 finding F-4), and `UpdatePaymentMethodIban` corrected to self-authorize mid-audit (finding F-1) after shipping on the same mistaken "CreateUser/UpdateUser/RequestEmailChange are unguarded" premise `ShippingZonePolicy`'s own actions were corrected for one story earlier — recorded as the third instance of that exact citation error. Updated the "sole outlier among **N** policies" bullets to ten and the `ProductVariantPolicy` hypothetical-absence ordinal to eleventh. Folded this footer's own `_Previously:` chain (`ShippingRatePolicy`, story 0036) into this single line, per [contracts.md](../contracts.md#doc-growth-management-rule)'s doc-growth-management rule — every fact that block recorded that still matters is already stated in the body's own per-section history; see git history for the full prior chain if needed._
