...
| Ability | Signature | Rule | Authorized from |
| --- | --- | --- | --- |
| `viewAny` | `(User $actor)` | holds `orders.view` | [`App\Livewire\Customers\Show::canViewOrderHistory()`](../../app/Livewire/Customers/Show.php) (story 0047) — not the originally-planned story 0055, which renders `OrderStatus` first too (see below) |
...
`VIEW_PERMISSION`/`CREATE_PERMISSION`/`EDIT_PERMISSION`/`DELETE_PERMISSION` are `public const` on the class, per [naming.md](../conventions/naming.md#permission-names)'s convention — the **eleventh** follower of it (`UserPolicy` remains the sole outlier among twelve policies).
...
- **The throwing guard is a wrapper around a non-throwing predicate**, the exact shape [step-up's own predicate/wrapper split](../security/step-up-authentication.md#one-predicate-two-shapes--the-ui-hint-must-call-the-guard-not-copy-it) already establishes for `EnsureRecentPasswordConfirmation::isRecentlyConfirmed()`. `TransitionOrderStatus`'s guard is a direct wrapper around `OrderStatus::isBackwardFrom()` (non-throwing), so a later UI hint (story 0055's "this will move the order backward" warning) asks the identical method the throw is built on and cannot drift from the rule that refuses.
...
_Last updated: 2026-09-17 — Story 0050 (Order manual cancellation backend). `OrderPolicy` gains a **sixth** ability, `cancel` — this app's first requiring TWO permissions (`orders.edit` **and** `orders.refund`, a human product decision, D-6) and the first whose boolean return genuinely depends on the target row (`$order->isManuallyCancellable()`). Added `OrderPolicy::ORDER_REFUND_PERMISSION` — the first single-name constant for `orders.refund` (0051's `RecordRefund` consumes it as a bare literal, per that story's own DR-2). Added [Manual order cancellation — the third state-based refusal, and the first ability requiring two permissions](#manual-order-cancellation--the-third-state-based-refusal-and-the-first-ability-requiring-two-permissions): `App\Exceptions\OrderCancellationBlockedException` (409, `OrderNotEditableException`'s shape), and the load-bearing nuance that distinguishes this instance from its two predecessors — the state clause lives in **both** the policy (as a UI hint, inert for a Super Admin) and the action (`CancelOrder`'s own direct throw, which binds one) — with the documented, accepted consequence that an ordinary actor's blocked-state attempt surfaces as a 403 rather than the 409 a Super Admin sees for the identical attempt, since `Gate::authorize('cancel', $order)` itself already refuses an ordinary actor before `CancelOrder`'s own checks are ever reached. The catalog's permission count (43) and `RolePermissionSeeder` are unchanged by this story — no new permission, `orders.edit` and `orders.refund` are both consumed exactly as already seeded.

_Previously: 2026-09-17 — Story 0051 (Order payment/refund state backend). The catalog grows from **42 to 43 permissions** and `Administrator`'s grant from 41 to 42, via a new `RolePermissionSeeder::ORDER_PERMISSIONS` constant (`orders.refund`) — the catalog's **first non-CRUD permission on a module that is not `roles`**, added without a new module slug or a new `ACTIONS` verb. `App\Actions\Orders\RecordRefund` gates on it with a bare `Gate::authorize('orders.refund')`, **not** a sixth `OrderPolicy` ability, despite 0045's own D-13 naming refunds as a candidate — see the new note under [`OrderPolicy` — the twelfth policy](#orderpolicy--the-twelfth-policy) for why the row-state refusal (payment status) stays a `ValidationException` inside the action rather than a policy method. All "N of 43"/"N of 42" counts on this page updated to match; historical "38"/"41"/"42" mentions describing an earlier story's own contemporary state are left as the dated facts they are.

_Previously: 2026-09-17 — Story 0049 (Order status transition backend). `OrderPolicy` gains a **fifth** ability, `transitionStatus` — reusing `EDIT_PERMISSION`, no new constant — with a real caller from day one (`App\Actions\Orders\TransitionOrderStatus`, a bare `Gate::authorize()`, not `LogRefusedPrivilegedAttempt`). Re-counted the ability table's caller roster directly against the table rather than trusting the 0048 footer's own claim: only `delete` has no caller today, correcting a stale "`viewAny` and `delete` remain uncalled" line that was already wrong the day story 0048 wrote it (story 0047, one day earlier, had already given `viewAny` a caller). Added [Order status regression confirmation — the app's second "are you sure" mechanism](#order-status-regression-confirmation--the-apps-second-are-you-sure-mechanism-and-why-it-is-not-step-up): `OrderStatusRegressionRequiresConfirmationException` (409), a per-call `$confirmed` parameter rather than step-up re-authentication, with a comparison table against `EnsureRecentPasswordConfirmation`/423 so a reader does not reach for the wrong mechanism by reflex.

_Previously: 2026-09-16 — Story 0048 (Order line-item editing backend). Added [Order editability — the second state-based refusal](#order-editability--the-second-state-based-refusal-and-the-shape-three-more-stories-copy): `App\Exceptions\OrderNotEditableException` (409), a direct `throw` from each of `AddOrderItem`/`RemoveOrderItem`/`UpdateOrderItemQuantity` when `orders.status` is `Shipped`/`Delivered`, checked strictly after `Gate::authorize('update', $order)` and re-verified a second time under `lockForUpdate()` — named as the shape stories 0050–0052 should each copy. Also corrected [What is deliberately not logged](#what-is-deliberately-not-logged)'s `RoleInUseException`/`ImmutableRoleException` bullet: Orders' own state-based refusal, unlike `Role`'s, **does** log. Earlier history (story 0045 and before) folded per [contracts.md](../contracts.md#doc-growth-management-rule) — see git history if needed._
