...
>
> **A polymorphic morph column is the same trap in a different shape.** `foreignUuid(...)` is the fix for an ordinary FK column pointing at a UUID-keyed table; `uuidMorphs(...)` (never the bare `morphs(...)`) is its counterpart for a polymorphic column like `notifications.notifiable_id` — see [A published stub is not the same thing as a package-vendored migration](#a-published-stub-is-not-the-same-thing-as-a-package-vendored-migration) below and [schema.md](schema.md#notifications).
