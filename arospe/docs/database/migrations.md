...
- [The three-way delete-behaviour rule: cascade, restrict, or null](#the-three-way-delete-behaviour-rule-cascade-restrict-or-null)
...
**Cascade — when the child has no independent meaning without its parent.** `order_items.order_id` is `cascadeOnDelete()` (**D-11**), the only cascade in either migration, for the identical reason [`create_passkeys_table`](#structure)'s `passkeys.user_id` already cascades: a line item recorded against no order is not a smaller version of a line item, it is nothing at all. Note this cascade is close to unreachable in practice today — orders are never hard-deleted this phase, `Cancelled` is a `status` value, not a delete — so it is insurance against a future hard-delete path rather than a live behaviour.

**Restrict — when the child is a peer whose own data would be destroyed by following the parent down.** `orders.customer_id`, `.payment_method_id`, `.sales_region_id` and `.shipping_rate_id` are all `restrictOnDelete()`, the same reasoning [`sales_regions.parent_id`](#uuid-primary-keys) already establishes: an order is not part of a customer, a payment method, a sales region or a shipping rate — it merely references one, and destroying any of those four rows must never silently take a historical order down with it (or, for `customer_id` specifically, silently orphan it — see [schema-other.md](schema-other.md#customers)'s own "orders are never orphaned" note, and story 0042's soft-delete-on-`Customer` decision that exists partly to make this restriction livable in practice).

**Null — when a snapshot preserves the row's meaning without the reference.** `order_items.product_id` and `.product_variant_id` are `nullOnDelete()` (**D-2**), matching [`media.uploaded_by`](schema-products.md#uploaded_by-and-the-soft-delete-interaction--read-this-before-fixing-the-fk)'s standing example. This one is **safe only because a snapshot already exists**: `product_name`, `product_sku` and `unit_price` are copied onto the line item at creation (see [schema-orders.md](schema-orders.md#the-price-snapshot--read-from-the-variant-never-the-parent-when-one-is-named)) and survive the null, so a line item whose catalog row was later deleted remains fully legible as history with a dangling reference — never a blank row. `nullOnDelete()` without that snapshot would be the wrong choice: an administrator retiring a discontinued product must never be blocked by history they cannot act on, but the history must still say what was actually ordered.

Verified against a live, migrated instance rather than read off either migration file: `php artisan db:table orders` reports `orders_customer_id_foreign`, `orders_payment_method_id_foreign`, `orders_sales_region_id_foreign` and `orders_shipping_rate_id_foreign` all as `no action / restrict`; `php artisan db:table order_items` reports `order_items_order_id_foreign` as `no action / cascade` and both `order_items_product_id_foreign` / `order_items_product_variant_id_foreign` as `no action / set null` — exactly the three-way split above, and no hand-written `$table->index()` on any of the six columns ([An FK column does not also get an explicit index here](#an-fk-column-does-not-also-get-an-explicit-index-here)).
