...
**Authorization**: gated by [`App\Policies\CustomerPolicy`](../../app/Policies/CustomerPolicy.php) (`viewAny`/`create`/`update`, all flat and tier-free, no per-target rule — see [architecture/authorization.md](../architecture/authorization.md#customerpolicy--the-tenth-policy-and-the-first-a-passive-record-with-no-privilege-tier-of-its-own)), on the already-seeded `customers.*` permissions (`RolePermissionSeeder::MODULES` has carried `customers` since task 0002 — no catalog change, no new permission, no re-seed).
