...
_Last updated: 2026-09-10 — Story 0038 (Payment methods — bank transfer with a validated IBAN, backend). Added the `payment_methods` table — a single-row-this-phase catalog carrying an administrator-configurable IBAN, validated for ISO 13616 structure plus the ISO 7064 mod-97 checksum — with column documentation, the three-layer "bank transfer is the only method" enforcement (including the documented Super Admin `Gate::before` exception to layer 2), the seeded re-seed guarantee, and the `UpdatePaymentMethodIban` self-authorization correction made during this story's own Phase 4 security audit (quoted per this project's audit-authored-page convention, since the task file's own draft reasoning did not hold). Updated the model count (thirteen → fourteen) and the UUID-status Notes bullets (seven → eight tables beyond ADR 0001's original seven), recording this as the one instance where an explicit user decision overrode a Three Amigos debate's `bigint` recommendation toward the standing UUIDv7 policy. No new ER-diagram entity — `payment_methods` has no relationships to anything yet, the identical starting shape `product_categories`/`shipping_carriers` shipped in._

_Previously: 2026-09-10 — Story 0036 (Shipping rate rules — backend). Added the `shipping_rates` table — per-carrier rate rules keyed on a zone and a weight bracket, with column/index documentation (verified live: exactly three indexes, no duplication — closing story 0033's own R-1 suspicion for real), the null-aware `scopeCoveringWeight()` bracket comparison, the deliberate absence of any uniqueness constraint, the deliberate absence of `SoftDeletes` and why it matters to the zone-delete guard's count, the two opposite-direction FK races (`1451` on zone delete, `1452` on rate create/update) and why each is narrowed rather than catching the whole `23000` class, and the self-authorizing rate actions. Added `SHIPPING_CARRIERS`/`SHIPPING_RATES` to the ER diagram and corrected `shipping_carriers`' own now-stale "earns no ER-diagram entry yet" sentence in place. The rate-precedence resolution algorithm this table's rows feed is domain behaviour documented in [architecture/shipping.md](../architecture/shipping.md), not repeated here. Folded the prior `_Previously:` line (story 0033, 2026-09-07) into this single line, per [contracts.md](../contracts.md#doc-growth-management-rule)'s doc-growth-management rule — no content changed or lost; see git history for the full prior chain if needed._
