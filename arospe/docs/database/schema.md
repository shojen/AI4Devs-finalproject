...
_Last updated: 2026-09-10 — Story 0043 (Customers — "new customer" notification, backend). Added the `notifications` table — Laravel's own `database`-channel storage, published via `notifications:table` and edited to use `uuidMorphs('notifiable')` rather than the stub's default `morphs('notifiable')`, since `users.id` is a `CHAR(36)` UUID and the stock column can never hold a real recipient. Added `USERS ||--o{ NOTIFICATIONS` to the ER diagram (the polymorphic recipient relation, the same shape `model_has_roles`/`model_has_permissions` already use) and a `NOTIFICATIONS` entity block. Recorded that a published `notifications:table` stub is app-owned source once in `database/migrations/`, not a package-vendored migration — the identical distinction [migrations.md](migrations.md#package-vendored-migrations) already draws for `create_permission_tables.php`, applied here to a second file. No model change (`App\Models\User` already carries `use Notifiable;`) and no new permission (`customers.view` was already seeded). Flagged explicitly that no notification-viewing UI exists yet, so `read_at` is written by nothing today — a recorded scope boundary, not a gap. Developed in parallel with story 0042 on separate branches; this reconciliation merge carries both stories' footers forward with no content lost.

_Previously: 2026-09-10 — Story 0042 (Customers — soft delete, backend). Added `customers.deleted_at` (its own alteration migration, `add_soft_deletes_to_customers_table`) and the new **Soft deletes, and why the `users` reasoning does not transfer** subsection — no `Customer::delete()` override, an email that stays reserved by decision rather than obfuscated, and why the `SoftDeletingScope`/Spatie-detach rules that bind `users` do not bind `customers`. Corrected the two now-stale claims this same story's own Definition of Done flagged: the `users`-only `SoftDeletes` claim under [`users`](#soft-deletes) and the "no `deleted_at`, deferred to 0042" cell under `customers`, both quoted rather than silently rewritten. Widened the `customers` Authorization line to name `delete()` as `CustomerPolicy`'s fourth ability. No ER-diagram change (`customers` still carries no FK either way) and no new model (`Customer` is edited, not added), so the model count and UUID-status Notes bullets are unchanged. Folded the prior `_Previously:` line (story 0041) into this single line, per [contracts.md](../contracts.md#doc-growth-management-rule)'s doc-growth-management rule — no content changed or lost; see git history for the full prior chain if needed._
