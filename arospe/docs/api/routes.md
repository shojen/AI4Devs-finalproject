...
The controller behind it is the repo's first domain controller — the convention it establishes (an HTTP boundary in front of an `app/Actions/` class) is documented in [conventions/base-standards.md](../conventions/directory-structure.md#controllers-sit-in-front-of-actions-not-instead-of-them).
...
_Last updated: 2026-09-23 — Story 0055 (orders list + detail/editor UI). Added `orders.index` and `orders.show` — the eleventh and twelfth permission-gated routes, numbered by the per-area list's own ordinals — to the route table, the `routes/` file list and the [Orders routes](orders.md) entry. Earlier changes folded per [contracts.md](../contracts.md#doc-growth-management-rule): 0057a moved the bell into a persistent topbar ("Layout-mounted Livewire components"), 0057 added that section, 0047/0044 added the Customers rows, and the per-area split created the linked files._
