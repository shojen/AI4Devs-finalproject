...
_Last updated: 2026-09-26 — Story 0063 (blog posts list + editor). Added `blog-posts.index`, `.create` and `.edit` to the route table, the `routes/` file list and the [Blog routes](blog.md) entry. Earlier changes are folded per [contracts.md](../contracts/token-and-doc-rules.md#doc-growth-management-rule): 0062/0060 added the blog tags and categories rows, 0055 the Orders rows, 0057a moved the bell into a persistent topbar, 0057 added the layout-mounted section, 0047/0044 added the Customers rows, and the per-area split created the linked files._
