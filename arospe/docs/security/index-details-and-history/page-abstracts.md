# Security Knowledge Base — Index details and history — Detailed page abstracts

> Part of [Security Knowledge Base — Index details and history](../index-details-and-history.md). **Read this part when:** you want the long narrative summary of what a security page established and which task/story raised it. The other parts are listed in the [hub](../index-details-and-history.md#parts).

## Detailed page abstracts
...
- [HTML sanitization](../html-sanitization.md) — the rules established by story 0024a's Phase 4 audit
  ...
  [conventions/base-standards.md](../../conventions/base-standards/livewire-and-flux-conventions.md#a-wireignored-client-owned-region--the-apps-first-instance)
...
- [Bounding an array of ids at the validation boundary](../array-validation-bounds.md) — the rule
  ...
  **Story 0028 is the rule's first real, shipped, closed call site.** `App\Livewire\Products\AttributeTypes\Index::save()`
  ...
  needs that the sales-region case did not. See [database/schema.md](../../database/schema-products/attribute-types-and-values.md#product_attribute_values).
...
- [Derived columns: every invariant must hold at every write site](../derived-column-invariants.md) — the
  ...
  **re-read the subject of the operation, not only the relation you authorize through**, the same
  ...
  the fix. That re-audit added two further sections. **[What the remediation introduced](../derived-column-invariants/retry-and-transaction-hazards.md#what-the-remediation-introduced-a-retried-transaction-is-a-retry-safe-unit-or-it-is-a-lost-update)**
  ...
  without fixing the closure first converts a rare loud 500 into silent data loss. **[Confirmed safe: `causedByConcurrencyError()` matches a message, not a class](../derived-column-invariants/retry-and-transaction-hazards.md#confirmed-safe-causedbyconcurrencyerror-matches-a-message-not-a-class)**
