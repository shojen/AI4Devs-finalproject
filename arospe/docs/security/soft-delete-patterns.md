...
See [database/schema.md](../database/schema-other.md#soft-deletes-and-why-the-users-reasoning-does-not-transfer) for the full column-level writeup, and [conventions/base-standards.md](../conventions/base-standards.md#deleting-a-user-goes-through-the-model-not-the-query-builder) for the corrected "only model using `SoftDeletes`" claim this story's own Definition of Done required fixing.
