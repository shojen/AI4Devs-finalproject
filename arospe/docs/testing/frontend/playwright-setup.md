...
_Last updated: 2026-09-15 — Story 0046 (Orders — "new order" notification, backend), Phase 5 code review, not itself a browser-testing change. Added a caveat directly under [Orphaned Playwright processes re-accumulate on every browser-test run in this environment](#orphaned-playwright-processes-re-accumulate-on-every-browser-test-run-in-this-environment): the plain `pkill -9 -f "playwright run-server"` cleanup command can match and kill its *own* invoking process when that invocation's visible command line (e.g. a `bash -c '...'` string) itself contains the literal substring `playwright run-server` — reproduced live by `code-reviewer` while running this story's full suite. The anchored form `pkill -9 -f "playwright[ ]run-server"` avoids the self-match without changing which real leaked processes it targets, and is now the form to default to when running the cleanup from inside any wrapping shell invocation rather than a bare interactive prompt. No other section changed by this pass — this story adds no new browser test, install step or CI workflow change.

_Previously: the chain of prior updates (2026-07-19 through 2026-09-06) covers, in order: the suite's initial install/wiring (Task 0006b) and its first three files (`LoginSmokeTest.php`, `UsersIndexTest.php` flat, `SalesRegionsIndexTest.php`); the "Waiting: one call is banned, one is bounded" rule and the icon-only-row selector ⚠️ (Task 0018); the fourth file `GalleryTest.php` plus three source-verified tooling limits — `->wait(n)` is not a polling primitive and races its own 5000 ms ceiling, a real file upload cannot be driven through `visit()`, and a page mounting one component twice duplicates every `data-test` hook (Story 0020); the fifth/sixth files (`WysiwygEditorTest.php`/`WysiwygEditorOutputHtmlTest.php`) and the orphaned-Playwright-processes finding this pass's caveat extends (Story 0021); the eighth file (`SearchableMultiSelectTest.php`), the DOM-occlusion diagnostic, and a corrected `RolesIndexTest.php` under-count (Story 0022); closing `GalleryTest.php`'s flaky-test ⚠️ to a ✅ via `retry(3, ..., 250)` (Story 0024b); and two same-day CI-only flake confirming instances plus a PHP-matrix correction (2026-09-06). Folded into this single line per [contracts.md](../../contracts.md#doc-growth-management-rule)'s doc growth management rule — no content, fact, or rule was changed or lost; see git history for the full prior chain if needed._
