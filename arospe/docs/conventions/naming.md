...
`App\Concerns\CustomerValidationRules` (story 0041) mirrors `UserValidationRules`/`ProfileValidationRules` exactly on its own docblock's own claim, but its four rule methods are **entity-prefixed** — `customerNameRules()`, `customerEmailRules()`, `customerPhoneRules()`, `customerAddressRules(string $prefix)` — for the identical collision-avoidance reason `ProductValidationRules` entity-prefixes its own: `ProfileValidationRules` already declares both `nameRules()` **and** `emailRules()`, and `ProductCategoryValidationRules` already declares `nameRules()` too, so a bare `nameRules()`/`emailRules()` here would fatal the moment a future consumer composes `CustomerValidationRules` alongside either. The prefix is applied uniformly across all four methods, not only the two that collide today — the same blanket-rather-than-selective choice `ProductValidationRules`'s own docblock defends, on the same reasoning (a rule reviewed in one glance is safer than a per-method judgement about which names might collide with a trait that does not exist yet). `customerAddressRules()` is this trait's own instance of `SalesRegionValidationRules`'s "a `<noun>Rules()` method's noun is the *field*, not the model" shape taken one step further: it takes a `$prefix` parameter (`'shipping'`/`'billing'`) and returns rules for **six** columns keyed by their full column name, so one method serves both address blocks rather than `customerShippingAddressRules()`/`customerBillingAddressRules()` duplicating the same six rule arrays twice.

**`CustomerValidationRules` is also the first `<Noun>ValidationRules` trait carrying a method that does *not* end in `Rules` and does *not* return a rule array — `normalizeCustomerAttributes()`.** This is a deliberate departure from the section's own opening rule ("every method on them is suffixed `Rules` and returns a rule array"), not an oversight: normalising a blank optional field to a real `null` — and uppercasing the two country codes — must happen **before** `Validator::make()` ever runs, since Laravel's validator skips every non-implicit rule for a blank string rather than rejecting it (see [errors-log.md](../errors-log.md#livewire-skips-convertemptystringstonulltrimstrings-and-laravel-skips-non-implicit-rules-for-a-blank-string--the-two-combine-to-let-a-raw--reach-a-decimal-column--2026-09-10)). That is a data transformation, not a validation rule, so it cannot be expressed as a `<noun>Rules()` method at all — naming it `normalizeCustomerAttributes()` rather than forcing it into the `Rules` pattern is what stops a reader mistaking it for one. The trait also carries a `public const OPTIONAL_FIELDS` array (the field list `normalizeCustomerAttributes()` iterates), the same "a constant, not a rule method" shape `MediaValidationRules::MAX_UPLOAD_KB`/`MAX_DIMENSION` already establish above — read as a trait, not the model that composes it: `CustomerValidationRules::OPTIONAL_FIELDS` cannot be read directly (PHP refuses a trait-constant reference), only through a consuming class, e.g. `App\Actions\Customers\CreateCustomer::OPTIONAL_FIELDS`.

**A note on the sequential numbering above, since this paragraph is the second to break it.** The "fourth"/"fifth"/"sixth"/"seventh" ordinals attached to `SalesRegionValidationRules`/`MediaValidationRules`/`ProductAttributeValidationRules`/`ProductVariantValidationRules` earlier in this section undercount by at least one each, because `ProductValidationRules` (story 0024) was introduced as "a deliberate, reasoned exception" with no ordinal assigned to it at all, and `ProductCategoryValidationRules`/`ShippingZoneValidationRules` were never numbered either. Rather than renumber every existing reference retroactively — which would itself go stale the next time a trait is added without updating this paragraph — this page states the trait count as a verified fact instead of a fragile running ordinal: **thirteen `<Noun>ValidationRules` traits exist in `app/Concerns/` as of story 0041** (verified by `ls app/Concerns/*ValidationRules.php`, not by trusting any running ordinal in this section). Cite a trait by name, not by position, going forward.
...
_Last updated: 2026-09-10 — Story 0041 (Customers CRUD backend). Added `App\Concerns\CustomerValidationRules` to the **Traits** section: entity-prefixed on all four rule methods (`customerNameRules()`/`customerEmailRules()`/`customerPhoneRules()`/`customerAddressRules()`) for the same `nameRules()`/`emailRules()` collision `ProfileValidationRules`/`ProductCategoryValidationRules` already claim, and the section's first trait carrying a non-`Rules`-suffixed method (`normalizeCustomerAttributes()`, a blank-to-`null` data transformation that must run before validation, not a rule) alongside a `public const OPTIONAL_FIELDS` (mirroring `MediaValidationRules`'s own constant-not-rule-method precedent). Corrected the section's trait count from twelve to **thirteen** (verified by `ls app/Concerns/*ValidationRules.php`). No new route name, permission, `Index`-in-a-subfolder instance or boolean-property shape this story.
