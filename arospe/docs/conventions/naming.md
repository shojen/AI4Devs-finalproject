...
_Last updated: 2026-09-17 — Story 0050 (Order manual cancellation backend). Added `App\Actions\Orders\CancelOrder` and `App\Exceptions\OrderCancellationBlockedException` to the **Classes** table. Extended the **Boolean properties** section with `App\Models\Order::isManuallyCancellable()` — the case where the adverb, not the noun, carries the meaning, since story 0052's auto-cancel deliberately never consults this predicate and a bare `isCancellable()` would become ambiguous the moment that story ships. No permission-catalog change: `orders.edit`/`orders.refund` are both consumed exactly as already seeded, and `OrderPolicy::ORDER_REFUND_PERMISSION` is the first single-name constant for `orders.refund` (0051's own literal is unchanged).

_Previously: 2026-09-17 — Story 0051 (Order payment/refund state backend). Added `RolePermissionSeeder::ORDER_PERMISSIONS` (`orders.refund`) to the **Permission names** section's quoted constants block — the catalog's first non-CRUD permission on a module other than `roles`, a different kind of amendment from `media`'s (no new module slug). Updated the module-labels ✅/❌ pair from "16 keys … covering 38 of the 42 permissions" to "17 keys … covering all 43 permissions", fixing forward a pre-existing drift (the denominator had been bumped to 42 after story 0019 added `media`, but the numerator was never updated to match).

_Previously: 2026-09-11 — Story 0044 (Customers — list + create/edit UI). Added `App\Livewire\Customers\Index` → `resources/views/livewire/customers.blade.php` as the sixth row in the **Livewire components and views** `Index`-in-a-subfolder exception table — the ordinary flat-path case, nothing new about the mechanism. No trait, permission, boolean-property or `trans_choice()` convention changed by this story.
