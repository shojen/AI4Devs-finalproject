...
Part of [Naming Conventions](naming.md) — see [naming.md](naming/classes.md#classes) for class/file naming and the rest of this project's naming conventions. This file covers the `<Noun>ValidationRules` trait-and-method naming convention on its own, since it had grown into the largest section of `naming.md`.
...
`App\Concerns\BlogTagValidationRules` (story 0059) is the first trait here with **two** methods for the same field: `nameFormatRules()` (format only) and `nameRules()` (format plus uniqueness), because its consumers disagree about what an existing name means — `CreateBlogTag`/`RenameBlogTag` refuse it, `FindOrCreateBlogTag` treats it as a hit. Like `BlogCategoryValidationRules` it declares a plain `nameRules()` (the same collision with `ProfileValidationRules` and `ProductCategoryValidationRules` described above, so it is composed alone), and shares its Unicode-aware `trimName()` and folded-length guard by *duplication*, not extraction: pulling them into a shared concern would have edited a shipped, sibling-owned file outside story 0059's scope, so the duplication is a recorded follow-up rather than an oversight.

`App\Concerns\BlogPostValidationRules` (story 0061) names every leaf method after the **field**, not the model — `titleRules()`, `blogCategoryIdRules()`, `statusRules()`, `bodyRules(BlogPostStatus $status)`, `publishedAtRules(BlogPostStatus $status, bool $enforceFuture = true)`, `tagNamesRules()`, plus the closure-returning `titleSlugRules(?string $ignoreBlogPostId)` — so none collides with the `nameRules()` of the two blog taxonomy traits if 0063's editor composes more than one. It is the first trait with **status-parameterised** rules: a `match` on the typed parameter, deliberately not `required_if`, because the status is an action argument rather than a key of the validated array and `required_if` would fail *open* when that key is absent. The actions hand it a string status validated with `Rule::enum()` (never a typed enum property, or a forged value is an uncaught `\ValueError`).

**A note on the numbering above.** Any ordinal attached to a trait in this section ("fourth", "seventh", …) is unreliable — several traits (`ProductValidationRules`, `ProductCategoryValidationRules`, `ShippingZoneValidationRules`) were introduced with no ordinal at all, silently invalidating every later count. Rather than renumber retroactively, this page states a verified fact instead: **seventeen `<Noun>ValidationRules` traits exist in `app/Concerns/` as of story 0059** (`ls app/Concerns/*ValidationRules.php`, recounted rather than incremented blind — the prior "thirteen … as of story 0041" count on this line was itself already one short even at 0041, since it did not count `PaymentMethodValidationRules` from the earlier story 0038). Cite a trait by name, not by position.
...
_Last updated: 2026-09-24 — Story 0061 (Blog posts — core CRUD backend). Added `BlogPostValidationRules` — the first trait with status-parameterised rule methods — and recounted the roster to eighteen (`ls app/Concerns/*ValidationRules.php`). `OrderValidationRules` (0045/0048) carries the array-bound constants and the scoped-existence rules described above._

_Earlier revision notes: [conventions--naming-validation-traits.md](../history/conventions--naming-validation-traits.md)._
