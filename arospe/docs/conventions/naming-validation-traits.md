...
_Last updated: 2026-09-24 — Story 0059 (Blog tags — backend). Added `BlogTagValidationRules` — the first trait with two methods for one field (format-only and format-plus-uniqueness) — and recounted the roster to seventeen (`ls app/Concerns/*ValidationRules.php`). `OrderValidationRules` (0045/0048) carries the array-bound constants and the scoped-existence rules described above._
