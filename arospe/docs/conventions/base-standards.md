...
_Last updated: 2026-09-10 — Story 0042 (Customers — soft delete, backend). `App\Models\Customer` now uses `SoftDeletes` (corrected the "the one model using `SoftDeletes`" claim under [Deleting a user goes through the model, not the query builder](#deleting-a-user-goes-through-the-model-not-the-query-builder), quoted rather than silently rewritten) — with **no** `delete()` override, unlike `User`'s. `CustomerPolicy`'s `Policies/` line entry updated: `delete()` is a fourth flat, tier-free ability, not the deferred "no `delete()` yet" 0041 left it at. No convention rule changed — a bare `use SoftDeletes;` and a flat policy method are both existing shapes, not new ones.
