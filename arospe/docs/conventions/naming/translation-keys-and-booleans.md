...
_Last updated: 2026-09-26 — Story 0063 (blog posts list + editor). Added the `BlogTags`, `BlogCategories` and `BlogPosts` `Index` rows and the `BlogPosts\Index`/`Editor` pair to [livewire-components-and-views.md](livewire-components-and-views.md), and replaced the "`blog_tags` is the second multi-word key" count with the list of multi-word registry keys shipped so far. Earlier history folded: 0055 added the `Orders\Index` row and the `isLineItemEditable()`/`isRefundable()` predicate notes; 0050 added `CancelOrder`/`OrderCancellationBlockedException` to the Classes table and the `isManuallyCancellable()` naming note; 0051 added `ORDER_PERMISSIONS` (`orders.refund`); 0044 added the Customers row; a doc-growth pass condensed the traits and permission-policy narratives. Each is described in its own section above._
