...
| `App\Livewire\BlogTags\Index` | `resources/views/livewire/blog-tags.blade.php` | ~~`resources/views/livewire/blog-tags/index.blade.php`~~ |
| `App\Livewire\BlogCategories\Index` | `resources/views/livewire/blog-categories.blade.php` | ~~`resources/views/livewire/blog-categories/index.blade.php`~~ |
| `App\Livewire\BlogPosts\Index` | `resources/views/livewire/blog-posts.blade.php` | ~~`resources/views/livewire/blog-posts/index.blade.php`~~ |
...
**Story 0063 is the fourth `Index`-plus-sibling pair, and the second `Index`/`Editor` one.** `App\Livewire\BlogPosts\Index` → `resources/views/livewire/blog-posts.blade.php` (flat, the table's last row, joined by the `BlogTags` and `BlogCategories` rows, whose screens have no sibling) and `App\Livewire\BlogPosts\Editor` → `resources/views/livewire/blog-posts/editor.blade.php` (the ordinary mirror rule, nested) repeat `Products/`'s shape. `Editor` backs two routes (`blog-posts.create` and `blog-posts.edit`), which does not change its view path.
